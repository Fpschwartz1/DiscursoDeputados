...
source("..\\03_2017JulAgo\\10_CorporaPorCriterios.R")
source("..\\03_2017JulAgo\\11_LimpaCorporaParaAnalise.R")
library(stringr)
library(dplyr)
...
# caso não exista corpora, deve-se gerar: geracorpora <- TRUE
geracorpora <- FALSE
if(geracorpora){
  for(ano in ini:fim){ 
    ...
    for(partido in lpartidos_aux){
      ...
      if(corpora_por_criterios(discursos, ano, ano, partido, c("GE", "PE"))){
        limpa_corpora(ano, ano, partido)  
      } 
...
for(ano in anos) {
  ...
  for(partido in lpartidos[[ano]]){
    ...
    arq <- paste0("..\\CorporaRDS\\corpora_", partido, "_", ano, "_", ano, "_limpo.rds")

    if(file.exists(arq)){
      ldfpa[[partido]] <- readRDS(arq) 
      if(nrow(ldfpa[[partido]]$bigramas) == 0){
...
plot(ini:fim, vico, type = "o", col = "red", ylab = "índice de similaridade", xlab = "ano")
lines(ini:fim, vicoa, type = "o", col = "blue")
lines(ini:fim, viopo, type = "o", col = "green")
grid()
```
